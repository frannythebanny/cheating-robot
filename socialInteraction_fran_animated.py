...
def handshake_2(nao_available=True):
    if nao_available:
        handshake()
        for i in range(1,4):
            move_arm_up()
...
# Start social interaction
    ...
    nao_speech(['''^start(handshake_2()) Hi! My name is Naomi \\pau=300\\ and I\'m a robot. ^wait(handshake_2())
                The closet back there ^start(pointing_closet())
                is my home! ^wait(pointing_closet()) ^start(postureProxy.goToPosture("StandInit", 0.7))
                In my free time I love to do sports.I\'m a 
    ...
    nao_speech(['Ah! Welcome, ' + name + ''' ! It\'s nice to meet you!
            I\'m very glad you\'re here! ^start(sad()) You know, 
            sometimes it can get a bit lonely down here. \\pau=500\\ Sometimes I don\'t see any humans for days
    and I\'m all by myself in the dark closet. ^start(postureProxy.goToPosture("StandInit", 0.7))
    But I really love humans! They are so much fun. \\pau=700\\ Do you want to hear a secret?'''],nao_available)
    ...
    if guess == 'Yes':
        ...
        It\'s hilarious.\\vol=80\\ \\pau=1000\\ Oh! I really like you ''' + name +
